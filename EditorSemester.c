...
static void update_starting_date_btn(GtkButton *button, Repo *repo) {
    GDateTime *time = g_date_time_new_from_unix_utc(repo_get_semester_start(repo));
    if (time == NULL) {
...
static void on_starting_date(GtkWidget *sender, gpointer user_data) {
    ...
    GtkWidget *calendar_label = GTK_WIDGET(gtk_label_new("Data rozpoczęcia semestru:"));
    gtk_grid_attach(grid, calendar_label, 0, 0, 1, 1);

    GDateTime *time = g_date_time_new_from_unix_utc(repo_get_semester_start(this->repo));
    if (time == NULL) {
        printf("Niepoprawna data rozpoczęcia semestru\n");
        return;
    }
    ...
    if (result == GTK_RESPONSE_OK) {
        gtk_calendar_get_date(calendar, (unsigned *)&year, (unsigned *)&month, (unsigned *)&day);
...
void editor_semester_show(EditorSemester *this) {
    GObject *starting_date_btn = gtk_builder_get_object(this->ui, "semester-starting-date");
    update_starting_date_btn(GTK_BUTTON(starting_date_btn), this->repo);
    g_signal_connect(G_OBJECT(starting_date_btn), "clicked", G_CALLBACK(on_starting_date), this);
