...
bool test_diagonals(state *s) {
    ...
    if (sum != s->expected) return false;
    sum = 0;
    for (int d = 0; d < s->n; d++) {
...
int test(state *s, int field, int value) {
    ...
    // pozycji
    if (s->restore) {
        if (s->tab[field] > value) return 1;
    }

    // szybkie odrzucanie
    if (value <= 0) return 1;
    if (value > s->size) return 1;
    if (s->used[value - 1]) return 1;
    ...
    if (col == 0) s->cur_row_sum = value;
    else s->cur_row_sum += value;
    s->cur_col_sums[col] += value;

    // odrzucanie
    if (s->cur_row_sum > s->expected) goto end;
    if (s->cur_col_sums[col] > s->expected) goto end;
    ...
    if (col_next == 0) remaining = s->expected;
