...
void dialog_week_summary(Repo *repo, Timestamp week_start) {
    ...
    ID ot_list_len = reservations_for_time_period(repo, week_start, week_end, &ot_list, INVALID_ID);
...
void dialog_availability_ranking(Repo *repo) {
    ...
    for (ID i = 0; i < eq_list_len; i++) {
        ...
        OneTimeReservation *ot_list;
        ID ot_list_len = reservations_for_time_period(repo, start, end, &ot_list, i);
